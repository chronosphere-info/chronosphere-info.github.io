...
series: 
- code: occs
  name: Basic denormalized occurrence table
- code: occs2
  name: Basic denormalized occurrence table with references
- code: occs3
  name: Basic denormalized occurrence table with references, updated with abundance
    data
...
<td>
{% if i.code == page.defaultSeries %}
<strong><a href="{{site.url}}{{site.basurl}}/data/pbdb/{{i.code}}/">{{i.name }}</a></strong> <i>(default)</i>
{% else %}
<a href="{{site.url}}{{site.basurl}}/data/pbdb/{{i.code}}/">{{i.name }}</a>
{% endif %}
</td>
