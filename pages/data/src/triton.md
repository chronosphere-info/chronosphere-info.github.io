...
permalink: /data/triton/
layout: page
src: triton
series: 
- code: full
  name: Full version, with duplicates
- code: no-rounding
  name: No-rounding
- code: main
  name: As in publication

description: null
sourceURL: https://springernature.figshare.com/collections/Triton_a_new_database_of_Cenozoic_Planktonic_Foraminifera/5242154/2
sourceShort: 
logo: Triton.png
defaultSeries: full
---
...
<td>
{% if i.code == page.defaultSeries %}
<strong><a href="{{site.url}}{{site.basurl}}/data/triton/{{i.code}}/">{{i.name }}</a></strong> <i>(default)</i>
{% else %}
<a href="{{site.url}}{{site.basurl}}/data/triton/{{i.code}}/">{{i.name }}</a>
{% endif %}
</td>
