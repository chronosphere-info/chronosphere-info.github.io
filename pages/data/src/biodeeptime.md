---
title: "BioDeepTime"
permalink: /data/biodeeptime/
layout: page
src: biodeeptime
series: 
- code: denormalized
  name: Denormalized biogeographic observations
- code: rdb
  name: SQLite Relational Database
- code: neotoma-bchron
  name: Bchron ages for records from the Neotoma Paleoecology Database

description: null
sourceURL: https://doi.org/10.5281/zenodo.7504616
sourceShort: 
logo: 
defaultSeries: denormalized
---
...
<td>
{% if i.code == page.defaultSeries %}
<strong><a href="{{site.url}}{{site.basurl}}/data/biodeeptime/{{i.code}}/">{{i.name }}</a></strong> <i>(default)</i>
{% else %}
<a href="{{site.url}}{{site.basurl}}/data/biodeeptime/{{i.code}}/">{{i.name }}</a>
{% endif %}
</td>
